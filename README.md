...
- The `TestProject/` folder contains the unit tests, integeration tests and coverage report of the source code and can be accessed using [this](TestProject/) link.
...
dotnet watch
```
3. If local server has to be run then open another terminal instance and run the server &rarr;
```bash
npm install # install dependencies
node server.js
```
4. However server is connected to mongoDB with a private connection string, so leaderboad cannot be accessed locally.

5. Within a few seconds, this should direct you to your browser, with locally running version of the game.
...
- **Jest**: Provides an easy-to-use interface for unit testing. It also includes built-in code coverage tools, allowing to track test coverage and ensure robust testing of application's logic.
...
dotnet test
```
To collect coverage, windows machine has to be used, run &rarr;
```shell
